...
$rows = $object->select(); // Get all géneros from the database
?>
<!DOCTYPE html>
<html lang="es">
...
<body>
                ...
                <table id="myTabla" class="table table-striped mb-0">
                    ...
                    <tbody>
                        <?php foreach ((array) $rows as $row) { ?>
                            <tr>
                                <td><?= $row['id_genero'] ?></td>
                                <td><?= $row['descrip_genero'] ?></td>
                                <td>
                                    <a href="edit.php?id_genero=<?= $row['id_genero'] ?>" class="btn btn-warning">Editar</a>
                                    <a class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#iddel<?= $row['id_genero'] ?>">Eliminar</a>
    ...
    <div class="container" id="ventana" style="display:none;">
        <div class="mb-3">
            <h2 style="font-size: 3.00rem;">Listado de Géneros</h2>
        </div>
        <div class="table-responsive table-scroll" data-mdb-perfect-scrollbar="true" style="position: relative; height: 700px;">
            <table class="table table-striped mb-0" style="font-size: 2.00rem;">
                ...
                <tbody>
                    <?php foreach ($rows as $row) { ?>
                        <tr>
                            <td colspan="1"><?= $row['id_genero'] ?></td>
                            <td colspan="3"><?= $row['descrip_genero'] ?></td>
                        </tr>
                    <?php } ?>
                </tbody>
            </table>
